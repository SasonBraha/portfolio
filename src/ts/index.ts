...
import './canvas.ts';
//@ts-ignore
import { areClipPathShapesSupported, isBackgroundClipTextSupported, copyToClipboard } from './utils.ts';
...
contactForm.onsubmit = e => {
  ...
  document.querySelectorAll('.contact__validation-error').forEach(el => el.remove());
  document.querySelectorAll('.contact__contentEditable').forEach(el => el.classList.remove('invalid'));
  ...
  // Init Fetch
  const fetchUri = process.env.NODE_ENV === 'development' ? 'http://localhost:3000' : 'https://personal-portfolio-inquiries.herokuapp.com/';
  fetch(fetchUri, {
    method: 'post',
    headers: {
    ...
    res.json().then(data => {
      ...
      if (!success) {
        ...
        Object.entries(errors).map(([ id, { message } ]) => {
          ...
          invalidInput.parentNode.insertBefore(errorSpan, invalidInput.nextSibling);
